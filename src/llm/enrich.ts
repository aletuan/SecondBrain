...
const DEFAULT_ENRICH_MAX_COMPLETION_TOKENS = 4096;
const MIN_ENRICH_MAX_COMPLETION_TOKENS = 256;
const MAX_ENRICH_MAX_COMPLETION_TOKENS = 32_000;

/** Upper bound for enrich completion length; invalid/unset → 4096. */
export function resolveEnrichMaxCompletionTokens(): number {
  const raw = process.env.ENRICH_MAX_COMPLETION_TOKENS?.trim();
  if (!raw) return DEFAULT_ENRICH_MAX_COMPLETION_TOKENS;
  const n = Number.parseInt(raw, 10);
  if (
    Number.isFinite(n) &&
    n >= MIN_ENRICH_MAX_COMPLETION_TOKENS &&
    n <= MAX_ENRICH_MAX_COMPLETION_TOKENS
  ) {
    return n;
  }
...
export async function buildEnrichmentSections(
  ...
  const res = await client.chat.completions.create({
    ...
    max_tokens: resolveEnrichMaxCompletionTokens(),
    messages: [
