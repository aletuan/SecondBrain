...
- Open to light layout or documentation changes that make the split obvious: `cli/src/` is the CLI ingest app; `reader-web/` is the separate reader UI package—without requiring a full monorepo rework.

## Learned Workspace Facts

- The repository is one project with two deliverables: the ingest CLI (root package + `cli/src/`) and the reader web app (`reader-web/`), which shells the CLI for ingest.
