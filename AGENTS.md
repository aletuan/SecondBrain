...
- When restructuring code or expanding tests, treat `src/vault/` as the Obsidian writer boundary—avoid invasive refactors there unless explicitly in scope.
- When adding missing critical tests, prefer test-driven development.
- Open to light layout or documentation changes that make the split obvious: root `src/` is the CLI ingest app; `reader-web/` is the separate reader UI package—without requiring a full monorepo rework.

## Learned Workspace Facts

- The repository is one project with two deliverables: the ingest CLI (root package + `src/`) and the reader web app (`reader-web/`), which shells the CLI for ingest; confusion is often naming (`src/` at repo root), not two unrelated products.
