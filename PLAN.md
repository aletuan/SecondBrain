...
- [ ] Viết checklist “post-ingest wiki” trong schema vault (mục 6).
...
- [ ] Viết `Wiki/SCHEMA.md` (hoặc tên bạn chọn): mục tiêu, cây thư mục, quy tắc link tới `Captures/`, checklist ingest/query/lint.
- [ ] Ghi rõ **mapping taxonomy/category** → ưu tiên file topic nào (bám `config/categories` hoặc taxonomy API nếu có).
- [ ] (Tuỳ chọn) Một đoạn ngắn trong `CLAUDE.md` trỏ tới file schema vault để session code biết chỗ tìm.
