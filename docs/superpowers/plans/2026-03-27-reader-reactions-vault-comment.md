...
**Goal:** Cho phép người đọc chấm 1–5 sao và ghi chú tùy chọn mỗi lần, lưu lịch sử append vào `{slug}.comment` (Markdown) trong thư mục capture; hiển thị và gửi qua reader API + UI cuối trang capture.
...
**Tech Stack:** TypeScript, `reader` Vite, Node `fs` trong vault service / middleware, Vitest cho parser.
...
| File | Vai trò |
|------|---------|
| `reader/vault/reactionsMarkdown.ts` (mới) | `parseReactionsMarkdown`, `appendReactionEntry`, hằng số template, `MAX_COMMENT_CHARS` |
| `reader/vault/service.ts` | Export `getCommentPath(captureDir)` hoặc dùng chung logic với `getCaptureFiles` |
| `reader/vault/apiMiddleware.ts` | `GET` + `POST` `/api/captures/:id/reactions` |
| `reader/src/types.ts` | Type `ReactionEntry` nếu cần chia sẻ |
| `reader/src/main.ts` | Section HTML trong `renderCaptureDetail`, bind sau load, gọi API |
| `reader/src/style.css` | Style khối reactions + timeline |
| `reader/tests/reactionsMarkdown.test.ts` (mới) | Parser + append round-trip |
...
**Files:**
- Create: `reader/vault/reactionsMarkdown.ts`
- Create: `reader/tests/reactionsMarkdown.test.ts`
...
- [ ] **Step 5:** Chạy `pnpm vitest run reader/tests/reactionsMarkdown.test.ts`

- [ ] **Step 6:** Commit `feat(reader): reactions markdown parse and append`
...
**Files:**
- Modify: `reader/vault/service.ts`
...
- [ ] **Step 3:** `pnpm exec tsc --noEmit` trong `reader`

- [ ] **Step 4:** Commit `feat(reader): resolve capture comment file path`
...
**Files:**
- Modify: `reader/vault/apiMiddleware.ts`
- May use: `reader/vault/reactionsMarkdown.ts`, `service.ts`
...
- [ ] **Step 5:** Cập nhật `reader/README.md` — hai endpoint mới (một đoạn trong API).

- [ ] **Step 6:** Commit `feat(reader): API for vault reactions file`
...
- [ ] **Step 6:** Commit `feat(reader): capture reactions UI and timeline`

---

### Task 5: Verification

- [ ] **Step 1:** `cd reader && pnpm exec tsc --noEmit`
- [ ] **Step 2:** `pnpm test` từ root repo (nếu có script chạy cả reader tests) hoặc `cd reader && pnpm exec vitest run`
