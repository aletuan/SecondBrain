...
**Tech Stack:** TypeScript, Vite, existing reader API (`reader/vault/service.ts`, `apiMiddleware.ts`), Vitest at repo root (`tests/**/*.test.ts`).
...
| File | Responsibility |
|------|----------------|
| `reader/vault/service.ts` | Add `categories: string[]` to list items (parse from `fm` via same rules as `parseCategoryList` in main — extract shared parser or duplicate minimal parse in service). |
| `reader/src/types.ts` | `CaptureListItem.categories?: string[]` (or required `[]`). |
| `reader/src/captureFilters.ts` (new) | Pure `filterCaptures`, `SourceFilter` type, helpers re-export or move `isYoutubeCapture`/`isXCapture`/`isThreadsCapture` from `main.ts` to avoid duplication. |
| `reader/src/main.ts` | `layoutShell`, remove `aside.side`; `bindRail` → `bindAppNav`; remove `setSideInner` + inline side HTML into `renderHome` / `renderCapturesTable` / `renderCaptureDetail` / digest renders; wire filter UI + re-render table; `route()` updates. |
| `reader/src/style.css` | `.app` 2-col grid; `.app-nav` sections; remove/adjust `.side`, `.rail` → nav styles; drawer nav mirror; responsive breakpoints. |
| `reader/tests/captureFilters.test.ts` (new) | Unit tests for `filterCaptures` AND/OR source classification helpers. |
...
**Files:**
- Modify: `reader/vault/service.ts` — `CaptureListItem` type + `listCaptures()` push `categories`
- Modify: `reader/src/types.ts` — mirror field for client
- Reference: `reader/src/main.ts` `parseCategoryList` — **either** import a tiny shared util into vault (**avoid** importing `main` from service) **or** copy the 10-line parse logic into `service.ts` / `reader/vault/noteFm.ts` (new) used by both
...
- [ ] **Step 2:** Update `reader/src/types.ts` to include `categories?: string[]` (or `categories: string[]` default `[]`).
- [ ] **Step 3:** Run `pnpm -C reader typecheck` from repo root — expect PASS.
...
- [ ] **Step 5:** Commit — `feat(reader): include categories in capture list payload`
...
**Files:**
- Create: `reader/src/captureFilters.ts`
- Create: `reader/tests/captureFilters.test.ts`
- Modify: `reader/src/main.ts` — remove duplicate helpers once moved (in Task 2 or 3)
...
- [ ] **Step 2:** Run `pnpm vitest run reader/tests/captureFilters.test.ts` — FAIL (module empty).
...
- [ ] **Step 4:** Run `pnpm vitest run reader/tests/captureFilters.test.ts` — PASS.
- [ ] **Step 5:** Commit — `feat(reader): add captureFilters for client-side list filtering`
...
**Files:**
- Modify: `reader/src/main.ts` — `layoutShell()`
- Modify: `reader/src/style.css` — `.app`, new `.app-nav`, remove `.side` column rules; adjust `#side-inner` usages (delete)
...
- [ ] **Step 4:** `pnpm -C reader build` — PASS.
- [ ] **Step 5:** Commit — `feat(reader): two-column shell with app-nav placeholder`
...
- Modify: `reader/src/style.css` — drawer content
...
- [ ] **Step 5:** Commit — `feat(reader): app nav sections and bindAppNav`
...
**Files:**
- Modify: `reader/src/main.ts` — `renderCapturesTable`, `route()` captures branch
...
- [ ] **Step 5:** Commit — `feat(reader): captures metrics in main and client filters`
...
**Files:**
- Modify: `reader/src/main.ts` — remove `sideHome`, `sideCapture`, `sideDigests`, `sideDigestDetail` (or keep as string helpers renamed to `mainHomeAsideHtml` etc.)
...
- [ ] **Step 6:** Commit — `refactor(reader): move former sidebar panels into main`
...
**Files:**
- Modify: `docs/reader.md` if it documents 3-column layout
- Run: `pnpm typecheck`, `pnpm -C reader typecheck`, `pnpm vitest run`, `pnpm -C reader build`

- [ ] **Step 1:** Update `docs/reader.md` (short paragraph: 2-col + filters).
- [ ] **Step 2:** Run full verify commands; fix regressions (masthead, theme switcher, capture detail).
- [ ] **Step 3:** Commit — `docs: reader two-column layout note`
