...
**Files:** delete `reader-web/vault/runDigestCli.ts`; modify `reader-web/vault/service.ts`, `reader-web/vault/apiMiddleware.ts`, `reader-web/vault/brainDotenv.ts`

- [ ] **Step 1:** Delete `reader-web/vault/runDigestCli.ts`.

- [ ] **Step 2:** In `reader-web/vault/service.ts`, remove `listDigests` and `getDigest` exports and implementations (only used by digest API).

- [ ] **Step 3:** In `reader-web/vault/apiMiddleware.ts`, remove imports of `getDigest`, `listDigests`, and `runDigestCli`. Remove route handlers for:
  - `POST /api/digest`
  - `GET /api/digests`
...
- [ ] **Step 4:** In `reader-web/vault/brainDotenv.ts`, adjust the file comment so it no longer implies digest children need the same dotenv as digest CLI (accurate one-line comment).

- [ ] **Step 4b:** Confirm no stale imports:

```bash
rg -n "runDigestCli|listDigests|getDigest" reader-web/
```
...
```bash
pnpm typecheck
cd reader-web && pnpm exec tsc --noEmit
```
...
```bash
git add -A
git commit -m "chore(reader-web): remove digest and challenge API routes"
```
...
```bash
git add -A
git commit -m "chore(reader-web): remove digest UI and redirect legacy routes"
```
...
**Files:** `reader-web/src/style.css`
...
```bash
rg -n "digest|nav-digest" reader-web/src/style.css
```
...
git commit -m "chore(reader-web): drop digest and challenge styles"
```
...
**Files:** `CLAUDE.md`, `README.md`, `docs/reader-web.md`, `reader-web/README.md`; optional `github-profile/README.md`
...
Triage: update **current** product docs (`README.md`, `CLAUDE.md`, `docs/reader-web.md`, `reader-web/README.md`, optional `github-profile/README.md`); skip historical `docs/plans/` unless misleading as current architecture.
