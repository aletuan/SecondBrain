...
**Architecture:** Chỉnh `ENRICH_SYSTEM_PROMPT` trong `cli/src/llm/enrich.ts`; thêm `resolveEnrichMaxCompletionTokens()` đọc `ENRICH_MAX_COMPLETION_TOKENS` (default 4096); truyền vào `chat.completions.create` cho `buildEnrichmentSections` only.
...
**Files:** `cli/src/llm/enrich.ts`, `cli/tests/llm/enrich.test.ts`
