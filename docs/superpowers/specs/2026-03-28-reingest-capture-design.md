...
**Liên quan:** `src/ingest/runIngest.ts`, `src/vault/writer.ts`, `src/cli.ts`, `reader-web/vault/runIngestCli.ts`, `reader-web/vault/apiMiddleware.ts`, `reader-web/src/main.ts` (capture detail).
